...
2. Set up the project as a Python package with `uv` (hint: use `uv init`, edit `pyproject.toml` where necessary)
3. Use `uv venv` to setup a virtual environment with Python 3.12 (*yes - a different version!*) and use `uv add pandas ipykernel` to add minimal dependencies for this project
