...
- Open PowerShell
- Install using the `uv` installer script in PowerShell:
...
- After installation you may see a PATH notice for `C:\Users\<you>\.local\bin`.
  - Let `uv` update your shell automatically with:

```powershell
uv tool update-shell
```

  - Or add it for the current session with:
...
## 5.2. Create a new virtual environment with python 3.13 (will be downloaded automatically if not installed).
